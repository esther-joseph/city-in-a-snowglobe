...
function App() {
  ...
  // ?cover=off means no cover at all, not merely no hold. The cover carries
  // its own WebGL canvas, so mounting it for half a second on every page load
  // spends a second GL context that the suite and the artwork capture have no
  // use for. Browsers keep only a handful of live contexts and drop the oldest
  // when that runs out.
  const [coverMounted, setCoverMounted] = useState(!launch.skipCover)
